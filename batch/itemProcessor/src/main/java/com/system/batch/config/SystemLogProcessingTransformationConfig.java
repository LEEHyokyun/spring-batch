...
public class SystemLogProcessingTransformationConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager transactionManager;
    private final ItemProcessor<SystemLog, SystemLog> executionerProcessor;

    @Bean
    public Job systemLogProcessingJob() {
        return new JobBuilder("systemLogProcessingValidatorJob", jobRepository)
    ...
    public Step systemLogProcessingStep() {
        return new StepBuilder("systemLogProcessingStep", jobRepository)
                ...
                .processor(executionerValidatingProcessor())
                .processor(systemLogTransformingProcessor())
    ...
    public static class systemLogValidator implements Validator<SystemLog> {

        @Override
        public void validate(SystemLog systemLog) throws ValidationException {
            // case 1
            if (systemLog.getType() == null) {
                throw new ValidationException("System Log Type is null");
            }

            // case 2
            if (systemLog.getMessage() == null) {
                throw new ValidationException("System Log Message is null");
            }
        }
    }

    @Bean
    public ItemProcessor<SystemLog, SystemLog> executionerValidatingProcessor() {

        ValidatingItemProcessor<SystemLog> processor = new ValidatingItemProcessor<>(new systemLogValidator());
    ...
    @Bean
    public ItemProcessor<SystemLog, SystemLog> systemLogTransformingProcessor() {
        return item -> {
            if(item.getMessage().contains("rm -rf")) {
                item.setMessage("SPECIAL CASE");
            }
