...
public class TerminationSkipListenerConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager transactionManager;

    @Bean
    public Job terminationRetryJob() {
        return new JobBuilder("terminationSkipListenerJob", jobRepository)
    ...
    public SkipListener<Scream, Scream> skipListener() {
        return new SkipListener<>() {
            ...
            public void onSkipInProcess(Scream scream, Throwable t) {
                System.out.println("onSkipInProcess Targetted Id: [" + scream.getId() + "]. Throwed message = " + t.getMessage());
            }

            @Override
            public void onSkipInWrite(Scream scream, Throwable t) {
                System.out.println("onSkipWrite Targetted Id: [" + scream.getId() + "]. Throwed message = " + t.getMessage());
            }
        };
    }
    ...
    public ListItemReader<Scream> terminationRetryReader() {
        return new ListItemReader<>(List.of(
            ...
            public Scream read() {
                ...
                if(scream.getId() == 1){
                    System.out.println("[ItemReader]: This target would throw the exception = " + scream);
                    throw new TerminationFailedException("throwed Exception : " + scream);
                }

                System.out.println("[ItemReader]: Item Reader is normally terminating target = " + scream);
                return scream;
            }
        };
    }
