...
public class TerminationRetryProcessorConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager transactionManager;

    @Bean
    public Job terminationRetryJob() {
        return new JobBuilder("terminationRetryProcessorJob", jobRepository)
        ...
        return new ItemProcessor<>() {
            ...
            public Scream process(Scream scream) throws Exception {
                ...
                if (scream.getId() == 3 && mercy < MAX_PATIENCE) {
                    mercy ++;
                    System.out.println(" -> Exception! scream : id : 3 could not be terminated..");
                    throw new TerminationFailedException("Exception is throwed = " + scream);
                } else {
